...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, batch, return_history=False, **kwargs):
        ...
        # allow early stopping of the denoising process via max_timesteps
        max_timesteps = kwargs.pop('max_timesteps', None)
        total_steps = self.num_timesteps if max_timesteps is None else min(int(max_timesteps), self.num_timesteps)

        for j in reversed(range(0, total_steps)):
...
class Trainer(object):
    ...
    def evaluate(self, milestone, tries=(10, 0), verbose=False, return_history=False,
                 save_log=False, run_all=False, run_only=False, resume_eval=False,
                 max_timesteps=None, **kwargs):
            ...
            for m, test_dl in enumerate(test_dls):
                ...
                for n, data in enumerate(test_dl):
                    ...
                    for k in range(tries[m]):
                        ...
                        start = time.time()
                        # pass max_timesteps through to diffusion sampler so we can truncate the chain
    ...
    def summarize_success_rate(self, i, success_list, count, succeeded_graph_indices, success_rounds,
        ...
        # profiling of average sample time is disabled
        ave_sample_time = None
        if i not in log:
            log[i] = {}
        log[i].update({
            'success': success_list,
            'success_rate': top1,
            'success_rate_top3': topk,
        })
        if send_wandb:
            if self.use_wandb:
                wandb.log({
                    f"Accuracy ({i} obj)": top1,
                    f"Top 3 Accuracy ({i} obj)": topk,
                })
