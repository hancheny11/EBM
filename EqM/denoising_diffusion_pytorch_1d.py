...
class GaussianDiffusion1D(nn.Module):
    def __init__(
        ...
        *,
        ...
        use_innerloop_opt_cross = False,
        show_inference_tqdm = True,
        enable_energy_plotting = True,
        baseline = False,
        sudoku = False,
        continuous = False,
        connectivity = False,
        ...
        self.use_innerloop_opt_cross = use_innerloop_opt_cross
        self.enable_energy_plotting = enable_energy_plotting
    ...
    def opt_step(self, inp, img, t, mask, data_cond, step=5, eval=True, sf=1.0, detach=True):
        ...
        # fwd0 -> bwd0 -> fwd1 -> bwd1 -> fwd2 -> bwd2 -> ... -> fwdN -> bwdN

        # print("[opt_step] Starting")
        ...
        with torch.enable_grad():
            for i in range(step):
                ...
                t1 = time.time()
                # print(f'Sample update time: {t1 - t0} seconds')

                # print(f'Comleted step {i}')

                if mask is not None:
                    img_new = img_new * (1 - mask) + mask * data_cond

                # Hack for cross-step verificaiton
                if self.continuous:
                    sf = 2.0
                else:
                    sf = 1.0
                ...
                energy_traj.append(energy.detach().mean().item())
        ...
        # for idx, e in enumerate(energy_traj):
        #     print(f"[opt_step] Step {idx}: energy = {e}")

        # print("t shape: ", t.shape)
        # self.energy_history[t.item()].append(energy_traj)

        return img, energy_traj


    def opt_step_cross(self, inp, img, t, mask, data_cond, step=5, eval=True, sf=1.0, detach=True):

        # print("[opt_step_cross] Starting")
        ...
        def process_new_img(img_new, img, energy):
            if mask is not None:
                img_new = img_new * (1 - mask) + mask * data_cond

            # Hack for cross-step verification
            if self.continuous:
                sf = 2.0
            else:
                sf = 1.0
        ...
        with torch.enable_grad():
            ...
            energy, grad = self.model(inp, img, t, return_both=True)
            img_new = img - extract(self.opt_step_size, t, grad.shape) * grad * sf  # / (i + 1) ** 0.5
            img = process_new_img(img_new, img, energy)

            for i in range(step):
                # print("[opt_step_cross] Step", i)
                # Store previous energy and gradient
                energy_prev, grad_prev = energy, grad

                # fwd -> bwd stays the same
                energy, grad = self.model(inp, img, t, return_both=True)

                # bwd -> fwd
                if i == 0:  # Use previous gradient to update image
                    img_new = img - extract(self.opt_step_size, t, grad.shape) * grad * sf  # / (i + 1) ** 0.5
                else:   # Use the gradient 2 steps ago
                    img_new = img - extract(self.opt_step_size, t, grad.shape) * grad_prev * sf  # / (i + 1) ** 0.5
        ...
        # for idx, e in enumerate(energy_traj):
        #     print(f"[opt_step_cross] Step {idx}: energy = {e}")

        return img, energy_traj
    
    
    @torch.no_grad()
    def plot_energy_history(self, savepath=None):
        import matplotlib.pyplot as plt
        plt.figure()
        for t_key, trajs in self.energy_history.items():
            for i, traj in enumerate(trajs):
                plt.plot(traj, label=f"t={t_key}#{i}")
        plt.xlabel("Step")
        plt.ylabel("Energy")
        plt.title("Energy trajectories per inference call")
    ...
    @torch.no_grad()
    def save_and_plot_energy_history(self, save_dir=None, prefix="energy_history"):
        """
        Plot and dump all recorded energy trajectories.

        self.energy_history has structure:
            { t_key: [ [E_step0, E_step1, ...],   # run 0
                    [E_step0, E_step1, ...],   # run 1
                    ... ] }

        Args:
            save_dir (str or None): Directory to save .png and .csv files.
                                    If None, only displays the plot.
            prefix (str): Filename prefix (default: 'energy_history')
        """
        ...
        if save_dir is not None:
            ...
            print(f"[plot_energy_history] Saved plot to {png_path}")
            plt.close()
        else:
            plt.show()
        ...
        if save_dir is not None:
            ...
            with open(csv_path, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["t", "run_id", "step", "energy"])
                for t_key, trajs in self.energy_history.items():
                    for run_id, traj in enumerate(trajs):
                        for step_id, energy_val in enumerate(traj):
                            writer.writerow([t_key, run_id, step_id, energy_val])
            print(f"[plot_energy_history] Saved CSV to {csv_path}")
    ...
    def p_sample_loop(self, batch_size, shape, inp, cond, mask, return_traj=False):
        ...
        for t in iterator:
            ...
            if self.use_innerloop_opt:
                if self.use_innerloop_opt_cross:
                    if t < 1:
                        img, energy_traj = self.opt_step_cross(inp, img, batched_times, mask, cond_val, step=step, sf=self.opt_sf)
                    else:
                        img, energy_traj = self.opt_step_cross(inp, img, batched_times, mask, cond_val, step=step, sf=self.opt_sf)

                else:
                    if t < 1:
                        img, energy_traj = self.opt_step(inp, img, batched_times, mask, cond_val, step=step, sf=self.opt_sf)
                    else:
                        img, energy_traj = self.opt_step(inp, img, batched_times, mask, cond_val, step=step, sf=self.opt_sf)

                img = img.detach()

                # Store energy trajectory for each timestep
                self.energy_history[timestep_idx].append(energy_traj)
                timestep_idx += 1
        ...
        if self.use_innerloop_opt:
            if self.use_innerloop_opt_cross:
                plot_prefix = 'cross_step_energy_history'
                print("[Cross-step]")
            else:
                plot_prefix = 'seq_step_energy_history'   
                print("[Sequential]")

        if self.enable_energy_plotting:
            self.save_and_plot_energy_history(save_dir='results', prefix=plot_prefix)
...
class Trainer1D(object):
    def __init__(
        ...
        *,
        ...
        # Update the diffusion model's energy plotting setting
        self.model.enable_energy_plotting = enable_energy_plotting
        ...
        # Whether to enable energy plotting
